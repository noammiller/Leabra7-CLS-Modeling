...
def tensorfy(old_num: int, new_num: int) -> torch.Tensor:
    x = torch.cuda.FloatTensor(8).zero_()
    if new_num != None:
        x[new_num] = 1
    if old_num != None:
        x[old_num] = 0.9
...
def pearson_correlation(patterns: Dict[int, torch.Tensor]) -> torch.Tensor:
    ...
    corr_matrix = torch.cuda.FloatTensor(dim, dim).zero_()
...
def test(networks: List[lb.Net]) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], float, float]:
    ...
    for layer in hidden_layers:
        im_sum[layer] = torch.cuda.FloatTensor(8, 8).zero_()
        fm_sum[layer] = torch.cuda.FloatTensor(8, 8).zero_()
